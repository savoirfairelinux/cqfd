...
However, it is recommended to switch to `external_docker_image` for fetching
from a distant registry, and use `custom_img_name` only if you want to specify a
custom Docker image name to be built by cqfd.

`external_docker_image` (optional): allows specifying a Docker image to pull from a
distant source. It is advised to use this variable instead of `custom_img_name`
if you want to use an external Docker image. For example, to use Ubuntu 26.04,
it can be set like:

```
external_docker_image=ubuntu:26.04
```
